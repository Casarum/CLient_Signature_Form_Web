...
def login():
    if request.method == "POST":
        ...
        if conn:
            try:
                cursor = conn.cursor()
                cursor.execute("SELECT username, role FROM perdoruesit WHERE username = ? AND password = ?", (input_username, input_password))
...
def search_client():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT emri, kodi FROM klient_furnitor WHERE emri LIKE ?", (f"%{search_text}%",))
